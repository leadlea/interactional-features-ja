#!/usr/bin/env bash
# 条件2（summary）のLLM Big5採点コマンド一覧
# 4教師 × 5trait = 20回の score_big5_bedrock.py 実行
# 手動ターミナル実行用（AWS Bedrock API経由）
#
# Usage: bash scripts/baseline/run_scoring_summary.sh

set -euo pipefail

MONO="artifacts/baseline/monologues_summary.parquet"
...
run_teacher() {
  ...
  for trait in $TRAITS; do
    out_dir="${OUT_BASE}/dataset=cejc_home2_hq1_v1__items=${trait}24__teacher=${teacher}__condition=summary"
    items_csv="${ITEMS_DIR}/items_ipipneo120_ja_${trait}24.csv"
    echo "=== ${teacher} / ${trait} (condition=summary) ==="
...
echo "✓ 条件2（summary）の全20回の採点が完了しました"
