...
def main():
    ...
    ap.add_argument("--exclude_cols", default="", help="comma-separated column names to exclude from X (but keep for filtering)")
    args=ap.parse_args()
    ...
    if len(d) < 5:
        raise SystemExit(f"Too few rows after filter: n={len(d)}. Lower thresholds.")
    ...
    # X columns
    Xcols=[c for c in d.columns if (c not in drop) and (c not in excl)]
    X=d[Xcols].apply(pd.to_numeric, errors="coerce")
